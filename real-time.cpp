...
short* ByteToShort(BYTE* input, int len) {
	// TODO Auto-generated method stub
	short *output = new short[len/2];
	for (int i = 0; i < len/2; i++) {
		BYTE bl = input[2 * i];
		BYTE bh = input[2 * i + 1];

		output[i] = (short) ((bh & 0x00FF) << 8 | bl & 0x00FF);
		/**
...
int8_t* ByteToInt(BYTE* input, int len) {
	// TODO Auto-generated method stub
	int8_t *output = new int8_t[len];
	for (int i = 0; i < len; i++) {
