...
vector<vector<float> > WaveFunction::getMFCCs(string filename){
    ...
    float *data = new float[FrmLen];
    float *FiltCoe1 = new float[FFTLen/2+1];
    float *FiltCoe2 = new float[FFTLen/2+1];
    int *Num = new int[FFTLen/2+1];
...
vector<vector<float> > WaveFunction::getMFCCs(short* input, int len){
    xishu.clear();
    SourceMFCCs.clear();
    //mfcc分析
    // mTWavHeader waveheader;
    ...
    // while(fread(buffer,sizeof(short),FrmLen,sourcefile)==FrmLen){
    int ptr = 0;
    while(ptr + FrmLen < len) {
        memcpy(buffer, input+ptr, FrmLen*2);
        // memcpy(buffer, input+ptr, FrmLen*2);
        HammingWindow(buffer,data);
        ComputeFFT(data,vecList);
        Filt(data, FiltCoe1, FiltCoe2, Num, En,vecList);
...
// new
// vector<vector<float> > WaveFunction::enframe(float* input, int len, int inc, int nx) {
//     int n = (nx-len+inc)/inc;
//     for (int i=0; i<n; i++) {
//         vector<float> temp;
//         for(int j=0; j<len; j++)
//             temp.push_back(input[i*inc+j]);
//         voice_frame.push_back(temp);
//     }
//     return voice_frame;
// }

// float* WaveFunction::filter(float* input, float* filter, int len) {
//     float* output = new float[len];
//     output[0] = input[0];
//     for (int i=1; i<len; i++) {
//         output[i] = filter[0]*input[i] + filter[1]*input[i-1];
//     }
//     return output;
// }

// void WaveFunction::endpoint(int16_t* input, int len) {
//     //幅度归一化到[-1,1]
//     int16_t input_max = 0;
//     for (int i=0; i<len; i++) {
//         if (abs(input[i]) > input_max) input_max = abs(input[i]);
//     }
//     float *data = new float[len];
//     for (int i=0; i<len; i++) {
//         data[i] = (input[i])*1.0/(input_max);
//     }
...
//     int maxsilence = 150;  // 15*10ms  = 150ms
//     int minlen  = 15;    // 15*10ms = 150ms
//     int status  = 0;
//     int count   = 0;
//     int silence = 0;

//     //计算过零率
//     vector<vector<float> > tmp1  = enframe(data, FrameLen, FrameInc, len-1);
//     vector<vector<float> > tmp2  = enframe(data++, FrameLen, FrameInc, len-1);
    
//     int height = (len-1-FrameLen+FrameInc)/FrameInc;
//     vector<vector<int> > signs, diffs;
//     for (int i=0; i<height; i++) {
//         vector<int> temp1, temp2;
//         for (int j=0; j<FrameLen; j++) {
//             temp1.push_back(tmp1[i][j]*tmp2[i][j] < 0);
//             temp2.push_back(tmp1[i][j]-tmp2[i][j] > 0.02);
//         }
//         signs.push_back(temp1);
//         diffs.push_back(temp2);
//     }
//     vector<int> zcr;
//     for (int i=0; i<height; i++) {
//         int temp = 0;
//         for (int j=0; j<FrameLen; j++) {
//             temp += signs[i][j]*diffs[i][j];
//         }
//         zcr.push_back(temp);
//     }

//     //计算短时能量
//     float coeff[2] = {1, -0.9375};
//     float *data_filt = filter(data, coeff, len);
//     vector<vector<float> > tmp3 = enframe(data_filt, FrameLen, FrameInc, len);
...
//             temp += fabs(tmp3[i][j]);
//         }
//         amp_max = max(fabs(temp), amp_max);
//         amp.push_back(temp);
//     }

//     //调整能量门限
//     amp1 = min(amp1, amp_max/4);
//     amp2 = min(amp2, amp_max/8);
